...
def class_command(update: Update, context: CallbackContext):  
    ...
    r = requests.get(LESSONS_URL.format(course_id=course_id), headers=headers)  
    for cls in r.json().get("todayclasses", []):  
        message = format_class_message(cls, COURSES.get(course_id, {}).get("name", "Unknown Course"))  
...
def authlist_command(update: Update, context: CallbackContext):  
    if update.effective_chat.id != CHAT_ID:  
        update.message.reply_text("❌ Unauthorized")  
        return  
    if not AUTH_USERS:  
    ...
    for uid, cid in AUTH_USERS.items():  
        course_name = COURSES.get(cid, {}).get("name", "Unknown Course")  
...
dispatcher.add_handler(CommandHandler("authlist", authlist_command))  
